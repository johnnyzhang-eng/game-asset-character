...
这一片锁的核心不是"能跑通",而是**三类静默错误**:
  ...
  ① **路线永不可达。** 这条真发生过:早先由 ``Render3DAdapter._key()`` 从
     ``CharacterCard.master_ref`` 反查资产,而 ``executor`` 建 card 时根本没给
     ``master_ref`` → 键恒为 None → 预检恒 False → 走真实 server 路径永远选不到三渲二。
     **而单元测试全绿**,因为它们直接构造了带 master_ref 的 card。所以本文件里最重要的
     那条用例是端到端的那条(``test_real_server_path_reaches_render3d_*``):
     它从"造型上有 model_3d_url"一路走到"渲帧策略真的被调用",中间不许有测试替身
     替代路线选择本身。
