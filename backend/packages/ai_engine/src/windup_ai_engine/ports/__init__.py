...
class ActionQuality:
    ...
    """分区动量(``slicing.quality.limb_motion``):``{区名: 动量}`` + ``spread`` / ``still``。

    上层拿它做的决定:某个**按动作语义该动的**区动量接近 0 → 那块网格没被骨骼驱动,
    多半是自动绑骨漏认了一条肢体,**别交付**,回上游重绑或换母版姿态。

    与 ``motion_scale`` / ``dead_frames`` 都不重复,补的是它们共同的盲区:
    那两个看的是**整幅**,而"腿在迈、手臂僵成柱子"整幅指标完全正常。
    2026-08-13 实测(四足走路序列帧):``motion_scale`` 11.08、死帧 0 —— 两道闸全绿,
    而前腿动量只有后腿一半、图上是根柱子。

    留空 dict 而不是 None:三渲二之外的路线也可以填,只是今天没有第二个填它的地方。
    """
