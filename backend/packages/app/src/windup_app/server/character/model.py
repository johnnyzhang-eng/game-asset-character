...
class CharacterOutfit(BaseModel):
    ...
    #
    # 为什么挂在造型一级而不是角色一级(#121):外观挂在造型上(每个造型自带 preview_url),
    # 角色级只有一张参考图,同一角色的不同造型共用不了一个 3D 模型。
    #
    # 为什么这个判断放在 DB 而不是做成引擎 port 上的一个查询(#122):"有没有 3D 资产"
    # 只有 DB 知道;让引擎回答它,要么引擎去反查存储(破"只吃 bytes"的分层),要么它只能猜。
    #
    # 建这份资产是**每造型一次性**的按次计费(图生 3D + 绑骨,约 ¥3.60),不在动作生成
    # 的请求路径上 —— 见 orchestrator.render3d_assets.Render3DAssetBuilder。
