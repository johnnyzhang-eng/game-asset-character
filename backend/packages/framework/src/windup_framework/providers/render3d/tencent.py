"""两个**按次计费** Protocol 的腾讯云混元实现(+ 一个 COS uploader)。
...
**花钱要有人点头**:``allow_spend`` 默认 ``False``,提交路径因此不可能被"顺手跑一下"触发。
开关放在构造而不是方法参数上,好让 Protocol 的方法签名不外泄"这条链路要花钱"。
"""
...
# 轮询中允许继续等的状态;不在表里的一律当失败,并把 JobId 写进异常供调用方再查(见
# :class:`JobFailedError`)。
_RUNNING = {"WAIT", "RUN", "RUNNING", "INIT", "PROCESSING", "QUEUING"}

PRESET_MOTIONS: Mapping[str, PresetMotion] = {
    # 只登记**实测过编号**的那些,没实测的不编名字,免得一个猜出来的名字被当成事实用下去。
    # 全集 1–48 见接口文档,要用别的直接传整数。
...
def _pick_artifact(files: list, want: str, *, strict: bool = True,
    ...
    ``strict=True`` 给返回裸 bytes 的调用方用:那边没有字段能标注真实格式,拿到 FBX 当 GLB
    用会让下游报 Bad glTF,症状伪装成"出帧管线坏了"。能如实标注 fmt 的调用方走
    ``strict=False`` 退到另一种格式 —— 硬要求只会让已经扣过费的产物取不回来。
    """
    def _fmt(f) -> str:
...
def _download(url: str, timeout: int = 600, tries: int = 3) -> bytes:
    ...
    重试安全:这是对成品 URL 的 GET,**不再扣积分**;不重试的代价是一次重新生成。
    长度校验挡的是"截断不抛异常" —— 服务端提前关流只给短 bytes,坏模型会一路流到出帧
    环节才暴露,在那儿看起来像"绑骨坏了"。异常文本一律过 :func:`redact`。
    """
    # URL 来自接口响应而不是我们拼的,而 urllib 照单全收 file:// 与内网地址且默认跟随跳转:
    # 上游被污染时,本地文件或元数据端点的内容会以"模型 bytes"的身份返回。
...
class TencentCosModelUploader(ModelUploader):
    ...
    key 用 **内容哈希**,同一份模型重传不堆副本、重试天然幂等。``expire`` 默认 2 小时:
    绑骨本身只要 40–60 秒,但排队时间不受我们控制,而 URL 失效等于任务白跑一次。
    """
...
class TencentModel3DProvider:
    ...
    背景会被一起建模,送检前把背景压白/透明。``face_count`` 默认压到 150000:接口默认
    500000 面出来的 GLB 常远超绑骨那边的 60MB 上限,少一趟"减面再来"的往返。
    """
    ...
    def build_params(self, master: bytes,
        ...
        ``extra_views`` 的**硬前提是各视图必须是同一个角色、同一姿势、同一尺度** ——
        侧/背视要以正面母版做参考图 i2i 出,各自文生等于送了三个人进去。
        """
        # 侧/背视超限同样会让整单被拒,而那时钱已经花了。
    ...
    def image_to_3d(self, master: bytes, *, want: str = "GLB",
        ...
        if self._ask_format:
            # 默认关:没有实测证据说 SubmitHunyuanTo3DProJob 收 ``ResultFormat``,而想要的
            # 格式已由取件端保证(按 Type 挑 + magic 复核)。发未验证的参数只多一份被网关
            # 拒的风险,换不到任何保证 —— 等谁实测过再默认打开。
...
class TencentAutoRigProvider:
    ...
    ``uploader`` **必填且无默认值**:构造不出一个"没有上传能力的绑骨 provider",免得跑到
    线上才发现模型送不出去。产出骨架见 :class:`RigInfo` —— 骨名无 ``mixamorig:`` 前缀,
    挂点按骨名寻址,去前缀后可直接复用既有握持参数。
    """
    ...
    def rig(self, model: bytes, *, want: str = "GLB",
        ...
        preset = self.resolve_motion(motion)           # 认不出的动作名要在花钱前炸
        if self._precheck:
            check_model(model)                         # 预检必须在上传与提交之前
    ...
    def fetch(self, job_id: str, *, want: str = "GLB") -> RiggedModel:
        """取一个**已完成**任务的产物。零成本:提交后的任何失败都不该让已扣的费作废。"""
    ...
    def _upload(self, model: bytes, fmt: str) -> str:
        ...
        if not isinstance(url, str) or not url.startswith(("http://", "https://")):
            # 不回显 url 全文:预签名 URL 里带 SecretId 与签名。
